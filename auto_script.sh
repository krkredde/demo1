...
# Step 4: Create the Pull Request and capture its URL
echo "Creating pull request from '$BRANCH_NAME' to '$BASE_BRANCH'..."
PR_URL=$(gh pr create --base "$BASE_BRANCH" --head "$BRANCH_NAME" --title "$PR_TITLE" --body "$PR_BODY" --assignee @me --label "automated" --draft -q '.url')
...
check_ci_status() {
  # Fetch the PR's checks and monitor their states (queued, in_progress, completed)
  CHECKS=$(gh pr checks "$BRANCH_NAME" --json state,workflow -q '.checkSuites.nodes[].checkRuns.nodes[] | {name: .workflow.name, state: .state, conclusion: .conclusion}')
